...
def create_access_token(data: dict) -> str:
    ...
    expire = datetime.utcnow() + timedelta(minutes=config.JWT_ACCESS_TOKEN_EXPIRE_MINUTES)  # JWT过期分钟
    token_data.update({"exp": expire})
    jwt_token = jwt.encode(token_data, config.JWT_SECRET_KEY, algorithm=config.JWT_ALGORITHM)  # 加入token
    return jwt_token
