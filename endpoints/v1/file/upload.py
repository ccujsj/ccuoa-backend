...
@upload_route.post("/new/upload/steamFile", deprecated=True,
                  ...
                  name="旧文件上传",dependencies=[Security(check_permissions, scopes=["staff"])])
async def INSECURE_upload_new_file(fileMIME: str, fileSize: int, fileName: str, file: bytes = File(...)):
    ...
    fileId = random_str()
    filePath = os.path.join(*config.SINGLE_FILE_STORAGE_PATH, fileId)
    info = FileMetaInfo(
        **(FileInfo(
            file_id=fileId,
            file_name=fileName,
...
@upload_route.post("/new/upload/file",
                  description="文件上传接口",
                  name="文件上传",dependencies=[Security(check_permissions, scopes=["staff"])])
...
@upload_route.get("/get/download/uid",
                  description="文件下载接口",
                  name="文件下载",dependencies=[Security(check_permissions, scopes=["staff"])])
